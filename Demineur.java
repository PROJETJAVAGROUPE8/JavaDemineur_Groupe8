...
public class Demineur
    extends JFrame implements MouseListener, WindowListener, ActionListener {
  ...
  private BoxLayout layoutPanneauHaut = new BoxLayout(panneauHaut,
      BoxLayout.LINE_AXIS);
  private Component box2; //Boxes utilisés dans le BoxLayout
  ...
  public void nouveau() {
    ...
    //on crée le bon nombre de mines puis on complète par des cases vides jusqu'à obtenir le nombre de cases total
    mines = "";
    ...
    for (int i = 0; i < HAUTEUR; i++) {
      for (int j = 0; j < LARGEUR; j++) {
        jeux[i][j].reset();
        jeux[i][j].removeMouseListener(this); //nécessaire pour éviter un bug lors de l'appel de nouveau() une 2eme fois
  ...
  private void jbInit() throws Exception {
    ...
    box2 = Box.createGlue(); //utilisé dans le jPanel1 pour la disposition
    box3 = Box.createGlue();
    ...
    //on le crée maintenant pour utiliser son GraphicsConfiguration()
  ...
  public void mouseReleased(MouseEvent e) {
    ...
    try {
      int x = (int) ( (JPanel) e.getSource()).getLocation().getX() + e.getX() +
          3; //génére des exceptions à cause du cast
      int y = (int) ( (JPanel) e.getSource()).getLocation().getY() + e.getY() +
          22;
  ...
  //déclenché par un appuie sur le bouton
  ...
  public void decouvre(int y, int x) {
    ...
    //Si on est au dessus d'un chiffre
      ...
      if (n == jeux[y][x].getChiffre()) { 
    	  //si il y en a autant que le nombre de mines autour, on découvre les 8 cases autour par un appel récursif de decouvre(int, int)
      ...
      for (int i = 0; i < HAUTEUR; i++) {
        for (int j = 0; j < LARGEUR; j++) {
          ...
          if (! (y == i && x == j) && mines.charAt(i * LARGEUR + j) == '1' &&
            ...
            //si il ya une mine, (recherche par rapport à la chaine mines
